    ...
    while True:
      ...
      ''')
        tipo_map = {
            ...
            '2': messages_pb2.DeviceType.CAMERA,
            '3': messages_pb2.DeviceType.TRAFFIC_LIGHT
        }
        ...
        if input_usuario == '1':
            ...
            ligar = True if ligar == '1' else False
            ...
            consultar = 'true'  # sempre manda como string

            if dispositivo in tipo_map:
                tipo = tipo_map[dispositivo]
                print(f"Enviando comando para consultar estado de {messages_pb2.DeviceType.Name(tipo)}")
                resposta_do_servidor = enviar_comando_para_gateway(comando, tipo, consultar=consultar)
                print(f"Resposta do Gateway: {resposta_do_servidor.decode('utf-8')}")
            else:
                print("Tipo de dispositivo inválido.")
